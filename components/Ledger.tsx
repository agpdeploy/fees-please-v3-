  ...
  return (
    <div className="animate-in fade-in duration-300 space-y-6 pb-20 relative">
      ...
      {activeTeamId && !isLoading ? (
        <>
          {/* 2. SEASON AUDIT CARD */}
          <div className="bg-white dark:bg-[#1A1A1A] border border-zinc-200 dark:border-zinc-800 rounded-3xl p-5 shadow-lg relative overflow-hidden transition-colors">
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <div className={`bg-zinc-100 dark:bg-zinc-800 rounded-xl px-3 py-1.5 text-sm font-black border border-zinc-200 dark:border-zinc-700 transition-colors ${overallNet >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
              ...
              <table className="w-full text-left text-[11px] font-bold">
                ...
                <tbody>
                  {seasonAudit.map((audit) => (
                    <tr key={audit.id} onClick={() => setSelectedFixture(audit)} className="border-b border-zinc-200 dark:border-zinc-800/50 last:border-0 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 cursor-pointer transition-colors">
          ...
          <div className="bg-white dark:bg-[#1A1A1A] border border-zinc-200 dark:border-zinc-800 rounded-3xl p-5 shadow-lg overflow-hidden transition-colors">
              ...
              {playerBalances.length === 0 ? (
                ...
                <>
                  {displayedDebtors.map((player) => (
                    <button key={player.id} onClick={() => openPlayerDetails(player)} className="w-full flex justify-between items-center bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 p-4 rounded-2xl transition-all group">
                      <span className="text-sm font-bold text-zinc-900 dark:text-white group-hover:text-emerald-500 transition-colors" style={{ '--tw-text-opacity': '1' } as any}>{player.name}</span>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-end px-1">
              ...
              <div className="relative w-1/2">
                <i className="fa-solid fa-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400 dark:text-zinc-700 text-[10px]"></i>
                <input 
                  type="text" 
                  placeholder="Filter Ledger..." 
                  value={searchTerm} 
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full bg-zinc-100 dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-800 rounded-full pl-8 pr-3 py-1.5 text-[10px] text-zinc-900 dark:text-white outline-none focus:border-zinc-400 dark:focus:border-zinc-700 transition-colors"
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              {filteredTransactions.slice(0, 30).map(tx => (
                <div key={tx.id} onClick={() => openPlayerDetails({ id: tx.player_id, name: tx.players ? `${tx.players.first_name} ${tx.players.last_name}` : 'Unknown', owed: 0 })} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-4 rounded-2xl flex justify-between items-center cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                  <div>
        ...
        <div className="text-center p-10 bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800/50 rounded-2xl transition-colors">
      ...
      {selectedFixture && (
        <div className="fixed inset-0 bg-black/20 dark:bg-black/80 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4 transition-colors">
          <div className="bg-white dark:bg-[#111] border border-zinc-200 dark:border-zinc-800 w-full max-w-[440px] rounded-3xl overflow-hidden flex flex-col max-h-[85vh] shadow-2xl animate-in slide-in-from-bottom-8 transition-colors">
            <div className="p-5 flex justify-between items-start border-b border-zinc-200 dark:border-zinc-800 transition-colors">
              <div>
              ...
              <button onClick={() => setSelectedFixture(null)} className="text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-colors"><i className="fa-solid fa-xmark text-xl"></i></button>
            </div>
            <div className="p-5 overflow-y-auto flex-1 space-y-2">
              {transactions.filter(tx => tx.fixture_id === selectedFixture.id).map(tx => (
                <div key={tx.id} className="bg-zinc-50 dark:bg-[#1A1A1A] border border-zinc-200 dark:border-zinc-800 p-4 rounded-xl flex justify-between items-center transition-colors">
        ...
        <div className="fixed inset-0 bg-black/20 dark:bg-black/80 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4 transition-colors">
          <div className="bg-white dark:bg-[#111] border border-zinc-200 dark:border-zinc-800 w-full max-w-[440px] rounded-3xl overflow-hidden flex flex-col max-h-[85vh] shadow-2xl animate-in slide-in-from-bottom-8 transition-colors">
            <div className="p-5 flex justify-between items-start border-b border-zinc-200 dark:border-zinc-800 transition-colors">
              ...
              <button onClick={() => {setSelectedPlayer(null); setIsManualModalOpen(false);}} className="text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-colors"><i className="fa-solid fa-xmark text-xl"></i></button>
            </div>
            ...
            {isManualModalOpen ? (
              <form onSubmit={handleManualSave} className="p-5 space-y-4 animate-in slide-in-from-right-4">
                <div className="flex bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-1 transition-colors">
                  ...
                  <button type="button" onClick={() => setManualType('fee')} className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-lg transition-colors ${manualType === 'fee' ? 'bg-zinc-200 dark:bg-zinc-800 text-red-500' : 'text-zinc-500'}`}>Charge (-)</button>
                </div>

                <select value={manualFixtureId} onChange={e => setManualFixtureId(e.target.value)} className="w-full bg-zinc-50 dark:bg-[#1A1A1A] border border-zinc-300 dark:border-zinc-800 rounded-xl px-4 py-3 text-sm text-zinc-900 dark:text-white outline-none font-bold transition-colors">
                ...
                <input type="number" placeholder="Amount ($)" value={manualAmount} onChange={e => setManualAmount(Number(e.target.value))} className="w-full bg-zinc-50 dark:bg-[#1A1A1A] border border-zinc-300 dark:border-zinc-800 rounded-xl px-4 py-4 text-center text-xl font-black text-zinc-900 dark:text-white outline-none focus:border-zinc-500 transition-colors" required />
                <input type="text" placeholder="Method / Note (e.g. Fine, Cash, Refund)" value={manualNote} onChange={e => setManualNote(e.target.value)} className="w-full bg-zinc-50 dark:bg-[#1A1A1A] border border-zinc-300 dark:border-zinc-800 rounded-xl px-4 py-3 text-sm text-zinc-900 dark:text-white outline-none focus:border-zinc-500 transition-colors" />
              ...
              <div className="p-5 overflow-y-auto flex-1 space-y-3">
                <button onClick={() => setIsManualModalOpen(true)} className="w-full py-4 border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-colors flex items-center justify-center gap-2 mb-2" style={{ color: themeColor }}>
                  <i className="fa-solid fa-plus"></i> Add Manual Transaction
                </button>
                {transactions.filter(tx => tx.player_id === selectedPlayer.id).map(tx => (
                  <div key={tx.id} className="bg-white dark:bg-[#1A1A1A] border border-zinc-200 dark:border-zinc-800 p-4 rounded-xl flex justify-between items-center transition-colors">
                    <div>
                ...
                {transactions.filter(tx => tx.player_id === selectedPlayer.id).length === 0 && (
                    <p className="text-center text-zinc-500 dark:text-zinc-600 text-[10px] uppercase font-black py-8 transition-colors">No history found.</p>
                )}
              </div>
            )}
          </div>
