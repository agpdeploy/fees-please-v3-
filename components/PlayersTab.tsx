...
export default function PlayersTab({ clubId, teams, players, loadClubData, showToast }: PlayersTabProps) {
  ...
  async function saveManualPlayer() {
    if (!firstName || !addTeamId) return showToast("First name and Team are required.", "error");
    setIsSaving(true);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.players && Array.isArray(data.players)) {
        // STRICT FALLBACKS ADDED HERE to prevent uncontrolled input warning
  ...
  const teamPlayers = players.filter(p => p.default_team_id === addTeamId);
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-left-4 fade-in">
      ...
      <div className="space-y-2">
        {teamPlayers.length === 0 && addTeamId && (
           <p className="text-center text-zinc-400 dark:text-zinc-600 text-[10px] uppercase font-bold py-4">No players in this team yet.</p>
        )}
        {teamPlayers.map(p => (
          <PlayerRow 
            key={p.id} 
...
function PlayerRow({ player, teams, loadClubData, showToast }: { player: any, teams: any[], loadClubData: () => Promise<void>, showToast: (msg: string, type?: 'success'|'error') => void }) {
  ...
  async function handleUpdate() {
    if (!editForm.first_name || !editForm.default_team_id) {
       return showToast("First name and Team are required", "error");
    }
    setIsSaving(true);
  ...
  const currentTeamName = teams.find(t => t.id === player.default_team_id)?.name || "Unknown Team";
