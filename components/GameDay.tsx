...
export default function GameDay() {
  ...
  // This catches the user when Square redirects them back to the web app
  useEffect(() => {
    ...
    if (dataString) {
      try {
        ...
        if (pendingTxStr && (response.status === 'ok' || response.transaction_id)) {
          const pendingTx = JSON.parse(pendingTxStr);
          
          // Save the successful transaction to Supabase
          supabase.from("transactions").insert([{ 
            player_id: pendingTx.player_id, 
            team_id: pendingTx.team_id, 
            fixture_id: pendingTx.fixture_id, 
            club_id: pendingTx.club_id,
            amount: pendingTx.amount, 
            transaction_type: 'payment', 
            payment_method: 'card' 
          }]).then(({ error }) => {
            if (!error) {
              showToast("Tap Payment Successful!", "success");
              // Adding to local state so UI updates instantly if data is already loaded
              setPaidPlayerIds(prev => [...prev, pendingTx.player_id]);
            }
          });
        } else if (response.status === 'error') {
          showToast("Payment Cancelled in Square", "error");
        }
        
        // Clean up the URL and storage so refreshing doesn't trigger it again
  ...
  // Fetch Team Settings & Current Fixture
  useEffect(() => {
  ...
  // Load Squad Data, Debts, and Payment Status
  ...
  const initiateTapToPay = (player: any) => {
    if (!process.env.NEXT_PUBLIC_SQUARE_APP_ID) {
      return showToast("Square App ID is missing in Vercel settings.", "error");
    }
    ...
    const amountCents = Math.round(grossAmount * 100); // Square requires cents

    // Store pending transaction to save upon return from the Square App
    ...
    // Build the payload for the Square POS App
    const data = {
      amount_money: {
        amount: amountCents,
        currency_code: "AUD",
      },
      callback_url: "https://feesplease.app/gameday", // MUST match the URL whitelisted in Square Dashboard
      client_id: process.env.NEXT_PUBLIC_SQUARE_APP_ID,
      version: "1.3",
      notes: `Match Fee: ${player.first_name} ${player.last_name}`,
      options: {
        supported_tender_types: ["CREDIT_CARD", "APPLE_PAY", "GOOGLE_PAY"]
      }
    };

    // Redirect to the native Square POS app
    const squareUrl = `square-commerce-v1://payment/create?data=${encodeURIComponent(JSON.stringify(data))}`;
  ...
  // Final Batch Save Logic (For Cash / Bulk Actions)
  async function processBatchPayments() {
    ...
    try {
      for (const player of selectedPlayers) {
        ...
        // Ensure we only process players who are still in cash mode (card mode handles itself)
        if (data.method !== 'cash') continue; 
      ...
      // Only clear the players we just saved (leaving any pending card players selected)
      const cashPlayerIds = selectedPlayers.filter(p => paymentData[p.id]?.method === 'cash').map(p => p.id);
      setSelectedPlayerIds(prev => prev.filter(id => !cashPlayerIds.includes(id)));
      
      setPayUmpire(false);
      showToast(`Saved Cash Payments!`);
    } catch (err) {
      ...
      {selectedPlayers.length > 0 && (
        <div className="space-y-4 animate-in slide-in-from-bottom-6 duration-300">
                ...
                <div key={player.id} className="bg-[#1A1A1A] border border-zinc-800 rounded-3xl p-5 shadow-lg relative animate-in fade-in">
                  ...
                  <div className="flex justify-between items-start mb-4 pr-6">
                    ...
                    <div className="flex bg-zinc-900 border border-zinc-800 rounded-xl p-1">
                      <button 
                        onClick={() => updatePlayerData(player.id, 'method', 'cash')} 
                      ...
                      <button 
                        onClick={() => {
                          ...
                          updatePlayerData(player.id, 'method', 'card');
          ...
          <div className="pt-4 space-y-4">
            ...
            {/* Batch save button now specifies it's for Cash payments if there are mixed methods */}
            <button 
              onClick={processBatchPayments} 
              disabled={isProcessing || selectedPlayers.filter(p => paymentData[p.id]?.method === 'cash').length === 0} 
